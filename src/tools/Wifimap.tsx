...
import { Wifi, LocateFixed, Search, Plus, Download, Upload, Eye, EyeOff, Trash2, Crosshair, X, MapPin } from 'lucide-react'
...
const pinHtml = (p: WifiPoint): string => {
  const c = AUTH_COLOR[p.auth ?? 'WPA2']
...
export default function Wifimap() {
  ...
  const [points, setPoints] = useState<WifiPoint[]>(() => [...loadUserPoints(), ...SEED_POINTS])
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return points
    return points.filter((p) => p.ssid.toLowerCase().includes(q) || p.place.toLowerCase().includes(q))
  }, [points, query])
  ...
  /* ── reconstruir marcadores (clusters por zoom, como la referencia) ── */
  useEffect(() => {
    ...
    if (z >= ZOOM_INDIVIDUAL) {
      ...
      for (const arr of grid.values()) {
        ...
        L.marker([lat, lon], {
          ...
          .bindTooltip(`${arr.length} red(es) — zoom para verlas`, { direction: 'top', offset: [0, -14] })
  ...
  const focusPoint = (p: WifiPoint) => {
    setSelected(p)
    setRevealed(false)
    mapRef.current?.flyTo([p.lat, p.lon], Math.max(ZOOM_INDIVIDUAL, mapRef.current.getZoom()))
  }
  ...
  const submit = () => {
    ...
    setPoints([...loadUserPoints(), ...SEED_POINTS])
    setFormOpen(false)
    ...
    try {
      ...
      const merged = [
        ...valid.map((p) => ({
          ...
          lat: p.lat as number,
          lon: p.lon as number,
          band: p.band as WifiPoint['band'] | undefined,
  ...
  const authTone = (a?: string): 'ok' | 'info' | 'warn' | 'bad' =>
    a === 'abierta' ? 'ok' : a === 'WEP' ? 'bad' : a === 'WPA3' ? 'info' : 'warn'
      ...
      <Reveal>
        <div className="card mb-4 flex flex-wrap items-center gap-2 p-4">
          <div className="relative min-w-52 flex-1">
            ...
            <TextInput value={query} onChange={(e) => setQuery(e.target.value)} placeholder="buscar SSID o lugar…" className="pl-9" />
          </div>
          ...
          <Button variant="ghost" onClick={exportJson} className="gap-2 px-3 py-2 text-xs"><Download size={14} /> JSON</Button>
      ...
      <Reveal>
        <div className="relative z-0 overflow-hidden rounded-2xl border border-edge">
          ...
          <div className="pointer-events-none absolute bottom-3 left-3 z-[500] flex flex-wrap gap-1.5 font-mono text-[10px]">
            ...
            {zoom < ZOOM_INDIVIDUAL && <span className="rounded-md border border-warn/50 bg-black/80 px-2 py-1 text-warn">acércate (zoom {ZOOM_INDIVIDUAL}+) para ver puntos individuales</span>}
          </div>
        </div>
      </Reveal>
        ...
        <Reveal>
          <div className="card mt-6 border-acento/30 p-6">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              {selected.band && <Badge tone="neutral">{selected.band}</Badge>}
              {selected.source === 'usuario' && <Badge tone="accent">tuya</Badge>}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="divide-y divide-edge/60 overflow-hidden rounded-xl border border-edge">
                ...
                <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                  <span className="font-mono text-[11px] uppercase tracking-wider text-grey">aportada</span>
                  <span className="font-mono text-[13px] text-ink">{selected.added} · {selected.author ?? 'anónimo'}</span>
                </div>
              </div>
          ...
          <div className="card mt-6 p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full font-mono text-xs">
                <thead><tr className="text-left text-grey/60"><th className="pb-2 pr-3">SSID</th><th className="pb-2 pr-3">lugar</th><th className="pb-2 pr-3">auth</th><th className="pb-2 pr-3">dist</th><th className="pb-2">clave</th></tr></thead>
                <tbody className="divide-y divide-edge/60">
                  {nearest.map(({ p, km }) => (
                    <tr key={p.id} className="cursor-pointer transition-colors hover:bg-acento/5" onClick={() => focusPoint(p)}>
                      ...
                      <td className="py-2 pr-3 text-grey">{p.place}</td>
                      ...
                      <td className="py-2 text-grey">{p.password ? (revealed ? p.password : '••••••') : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
